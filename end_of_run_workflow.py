...
def slack(func):
    ...
    Send a message to mon-prefect-cdi slack channel with the flow-run status.
    ...
    def end_of_run_workflow(stop_doc, api_key=None, dry_run=False):
        ...
        try:
            ...
            # Send a message to mon-prefect-cdi if flow-run is successful.
            message = f":white_check_mark: {CATALOG_NAME} flow-run successful. (*{flow_run_name}*)\n ```run_start: {uid}\nscan_id: {scan_id}```"
            mon_prefect_cdi.notify(message)
            return result
        except Exception as e:
            tb = traceback.format_exception_only(e)

            # Send a message to mon-prefect-cdi, mon-prefect if flow-run failed.
